...
func (m *MongoStore) ListUpcomingScheduledGames(after time.Time) ([]*ScheduledGame, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	cursor, err := m.scheduled.Find(ctx, bson.M{
		"scheduled_at": bson.M{"$gt": after},
	})
	if err != nil {
	...
	for cursor.Next(ctx) {
		...
		upcoming = append(upcoming, &sg)
	}
	return upcoming, cursor.Err()
}
